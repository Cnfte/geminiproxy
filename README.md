...
### 重要提示
- 若选择Nginx作为反向代理服务器，请务必提前准备好SSL证书。
- 安装完成后，请确保您的域名已正确解析至服务器的公网IP地址。
- 本脚本仅提供Gemini的反向代理功能，不负责将Gemini的API格式转换为OpenAI格式。
...
- 在出现的管理界面中，输入数字1选择安装反代。
- 脚本将引导您输入必要信息，包括您的域名、映射端口以及SSL证书获取方式（NAT服务器用户请务必选择第4种SSL证书获取方式）。  
  ![输入安装所需信息](img/5.jpg)  
  图5：按提示输入域名、端口及SSL配置
